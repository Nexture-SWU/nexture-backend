    ...
    def create_final_report(self, llm, user_uuid: str, chat_id: str):
        ...
        for attempt in range(1, max_retries + 1): 
            try:
                ...
                [출력 JSON 형식] 
                {{ "summary_accuracy": 숫자, # 줄거리와 학생요약이 일치하는지
                "expression": 숫자, # 표현력이나 문장 구성이 풍부한지, 자신의 감정을 잘 드러냈는지 
                "logical_thinking": 숫자, # 논리적 사고력을 가지고 있는지, 구조가 잘 잡혀있는지, 논리적 비약이 없는지 
                "manner": 숫자, # 독서 감상 대화에 성의를 가지고 임했는지, 말투가 적절했는지, 독서 감상문의 길이가 충분히 긴 지 
